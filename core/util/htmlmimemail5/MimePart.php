...
class Mail_MIMEPart
{
     ...
     *            multipart/mixed
     *            encoding - The encoding to use, 7bit, 8bit, base64, or
     *            quoted-printable
     *            cid - Content ID to apply
     *            disposition - Content disposition, inline or attachment
     *            dfilename - Optional filename parameter for content
     *            disposition
    ...
    public function encode () {
        ...
        if (! empty($this->subparts)) {
            ...
            $encoded['body'] = '--' . $boundary . MAIL_MIMEPART_CRLF . implode('--' . $boundary . MAIL_MIMEPART_CRLF, $subparts) . '--' . $boundary . '--' .
                     MAIL_MIMEPART_CRLF;
                } else {
             ...
             *
             * @param $body The
             *            body of the subpart, if any.
             * @param $params The
             *            parameters for the subpart, same
             ...
             *            data to encode.
             * @param $encoding The
             *            encoding type to use, 7bit, base64,
             *            or quoted-printable.
             */
             ...
             * @param $input The
             *            data to encode
             * @param $line_max Optional
             *            max line length. Should
            ...
            private function quotedPrintableEncode ($input, $line_max = 76) {
                ...
                while (list (, $line) = each($lines)) {
                    ...
                    for ($i = 0; $i < $linlen; $i ++) {
                        ...
                        if (($dec == 32) and ($i == ($linlen - 1))) { // convert
                                                                     // space at
                                                                     // eol only
                            $char = '=20';
                        ...
                        if ((strlen($newline) + strlen($char)) >= $line_max) { // MAIL_MIMEPART_CRLF
                                                                               // is
                                                                               // not
                                                                               // counted
                            $output .= $newline . $escape . $eol; // soft line
                                                                   // break; "
                                                                   // =\r\n" is
                                                                   // okay
                            $newline = '';
