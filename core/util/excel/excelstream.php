...
class xlsStream
{
    ...
    function _detect () {
        ...
        switch (pack('L', $abyz)) {
            ...
            // Compare the value to the same value converted in a Big-Endian
            // fashion
            case pack('N', $abyz):
                $this->endian = "big";
                break;
     ...
     *            stream open mode
     * @param $options (int)
     *            stream options (STREAM_USE_PATH |
     *            STREAM_REPORT_ERRORS)
     * @param $opened_path (string)
     *            stream opened path
     */
    function stream_open ($path, $mode, $options, &$opened_path) {
        ...
        // @TOXDO: test for invalid mode and trigger error if required
     ...
     *
     * @toxdo modify this to convert an excel stream to an array
     * @param $byte_count (int)
     *            number of bytes to read (in 8192 byte blocks)
     */
     ...
     * @access private
     * @param $row (int)
     *            worksheet row number (0...65536)
     * @param $col (int)
     *            worksheet column number (0..255)
     * @param $val (mixed)
