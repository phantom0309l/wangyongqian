<?php
/*
 * plain_sasl_client.php @(#) $Id: plain_sasl_client.php,v 1.2 2004/11/17
 * 08:00:37 mlemos Exp $
 */
        ...
        if ($status == SASL_CONTINUE) {
            switch ($this->credentials["mode"]) {
                ...
                case SASL_PLAIN_EXIM_DOCUMENTATION_MODE:
                    ...
                    break;
                default:
                    $message = $this->credentials["user"] . "\0" . $this->credentials["user"] .
                             (strlen($this->credentials["realm"]) ? "@" . $this->credentials["realm"] : "") . "\0" . $this->credentials["password"];
                            break;
                    }
            ...
            Function Step (&$client, $response, &$message, &$interactions) {
                switch ($this->state) {
                     ...
                     * switch($this->credentials["mode"]) { case
                     * SASL_PLAIN_EXIM_MODE:
                     * $message=$this->credentials["user"]."\0".$this->credentials["password"]."\0";
                     * break; case SASL_PLAIN_EXIM_DOCUMENTATION_MODE:
                     * $message="\0".$this->credentials["user"]."\0".$this->credentials["password"];
                     * break; default:
                     * $message=$this->credentials["user"]."\0".$this->credentials["user"].(strlen($this->credentials["realm"])
                     * ? "@".$this->credentials["realm"] :
                     * "")."\0".$this->credentials["password"]; break; }
                     * var_dump($message); $this->state=SASL_PLAIN_STATE_DONE;
                     * break;
                     */
