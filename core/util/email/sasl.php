<?php
/*
 * sasl.php @(#) $Id: sasl.php,v 1.11 2005/10/31 18:43:27 mlemos Exp $
 */
 ...
 * <package>net.manuellemos.sasl</package> <version>@(#) $Id: sasl.php,v 1.11
 * 2005/10/31 18:43:27 mlemos Exp $</version> <copyright>Copyright � (C) Manuel
 * Lemos 2004</copyright> <title>Simple Authentication and Security Layer
 * client</title> <author>Manuel Lemos</author>
 * <authoraddress>mlemos-at-acm.org</authoraddress> <documentation>
 * <idiom>en</idiom> <purpose>Provide a common interface to plug-in driver
 * classes that implement different mechanisms for authentication used by
 * clients of standard protocols like SMTP, POP3, IMAP, HTTP, etc.. Currently
 * the supported authentication mechanisms are: <tt>PLAIN</tt>, <tt>LOGIN</tt>,
 * <tt>CRAM-MD5</tt>, <tt>Digest</tt> and <tt>NTML</tt> (Windows or
 * Samba).</purpose> <usage>.</usage> </documentation> {/metadocument}
 */
...
*/
    ...
    /*
     * {metadocument} <variable> <name>mechanism</name> <type>STRING</type>
     * <value></value> <documentation> <purpose>Store the name of the mechanism
     * that was selected during the call to the
     * <functionlink>Start</functionlink> function.</purpose> <usage>You can
     * access this variable but do not change it.</usage> </documentation>
     * </variable> {/metadocument}
     */
    var $mechanism = '';

    /*
     * {metadocument} <variable> <name>encode_response</name>
     * <type>BOOLEAN</type> <value>1</value> <documentation> <purpose>Let the
     * drivers inform the applications whether responses need to be
     * encoded.</purpose> <usage>Applications should check this variable before
     * sending authentication responses to the server to determine if the
     * responses need to be encoded, eventually with base64 algorithm.</usage>
     * </documentation> </variable> {/metadocument}
     */
...
*/
    ...
    /*
     * {metadocument} </do> </function> {/metadocument}
     */
	...
	Function Start ($mechanisms, &$message, &$interactions) {
        ...
        for ($m = 0; $m < count($mechanisms); $m ++) {
            ...
            if (IsSet($this->drivers[$mechanism])) {
                ...
                $className = $this->drivers[$mechanism][0];
...
/*
 * {metadocument} </class> {/metadocument}
 */
