...
class AssertException extends BizException
{
    ...
    public function __construct ($message, $errorCode, $logMessage = '') {
        parent::__construct($message, $errorCode);
        if ($logMessage) {
            Debug::errlogEx($this, 'Ex', true, $logMessage);
        }
    }
}
...
class DBC
{
    ...
    public static function requireTrue ($express, $message = "", $errorCode = -1, $logMessage = '') {
        if (self::$needDBC <= 0 && false == $express) {
            if (Debug::canShowDebugInfo()) {
                Debug::errlogStackPoint($message);
            }
            $exception = new AssertException($message, $errorCode, $logMessage);
            throw $exception;
        }
    }
