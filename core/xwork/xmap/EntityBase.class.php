...
class EntityBase
{
    ...
    protected function init_database () {
        $defaultdb = DaoBase::get_defaultdb_name();
    ...
    public static function arraystrtolower ($row) {
        if (empty($row)) {
            return array();
        }
        $newRow = array();
    ...
    public function __construct (array $row, $id = 0, $isNewEntity = false, $dbconf = array()) {
        ...
        // 数组value转小写
        $this->_keys = self::arraystrtolower($this->_keys);
        $this->_keys_lock = self::arraystrtolower($this->_keys_lock);
        $this->_keys_abandon = self::arraystrtolower($this->_keys_abandon);
        ...
        // 注册实体
        BeanFinder::get("UnitOfWork")->registerEntity($this);
    }
    ...
    public function __get ($key) {
        ...
        if (method_exists($this, $method)) {
            ...
            // 未关闭日志
            if (false == XContext::getValueEx('EntityBase::__get-debug-close', false)) {
                $entity_class = get_class($this);
                if ($entity_class != 'WxUser') {
                    Debug::warn("[EntityBase::__get][{$entity_class}][{$key}]");
                }
            }
        ...
        if (isset($this->_belongtos) && isset($this->_belongtos[$key])) {
        ...
        if (! empty($belongto)) {
            ...
            if (isset($belongto['daotype']) && $belongto['daotype'] == 'XMapper') {
                $daotype = 'XMapper';
            }
    ...
    public function __set ($key, $value) {
        ...
        DBC::requireTrue(BeanFinder::get("UnitOfWork")->isReadOnly() == false, "data isReadOnly, {$this->getClassName()}::$key = $value");
        DBC::requireTrue(BeanFinder::get("UnitOfWork")->isCommited() == false, "data isCommited, {$this->getClassName()}::$key = $value");

        // 尝试重新注册loadMap
        BeanFinder::get("UnitOfWork")->tryReRegisterIfNeed($this);
        ...
        if (isset($this->_keys_lock)) {
            DBC::requireTrue(! in_array($key, $this->_keys_lock), "$key in _keys_lock");
        }
    ...
    public function remove () {
        DBC::requireTrue(BeanFinder::get("UnitOfWork")->isReadOnly() == false, "data isReadOnly , can't remove");
        DBC::requireTrue(BeanFinder::get("UnitOfWork")->isCommited() == false, "data isCommited , can't remove");
    ...
    public static function createID () {
        $idGen = BeanFinder::get('IDGenerator');
        return $idGen->getNextID();
    }

    // 获取一个实体, 静态绑定函数
    public static function getById ($id, $dbconf = array()) {
        $entityClassName = get_called_class();
        return Dao::getEntityById($entityClassName, $id, $dbconf);
    }

    // 批量转换
    public static function entitys2IphoneArrays ($entitys) {
        $arr = array();
