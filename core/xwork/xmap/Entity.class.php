...
class Entity extends EntityBase
{
    ...
    public function setDirty () {
        if ($this->currentVersion != $this->originalVersion) {
            throw new Exception('can not set dirty');
        }
    ...
    public function tryReload () {
        ...
        $dbExecuter = BeanFinder::get("DbExecuter", $this->getDatabaseName());
        ...
        if ($unitOfWork->needReload($this) && false == Config::getConfig("closereload") && true == $dbExecuter->isRwSplit()) {
            ...
            $tableName = DaoBase::getTableNameImp($this->getClassName(), $this->tableno, $this->getDatabaseName());
            ...
            if ($this->originalVersion != $row['version']) {
                Debug::warn("[-- tryReload " . $this->getClassName() . " : " . $this->id . " version( {$this->originalVersion} => {$row['version']} ) --]");
            }
    ...
    // 默认:status ->字符串, 带颜色
    public function getStatusStrWithColor () {
        return XConst::status_withcolor($this->status);
    }

    // 默认:status ->字符串
    public function getStatusStr () {
        return XConst::status($this->status);
    }

    // 逆序获取评论数据
    public function getComments ($cnt = 10, $offsetcommentid = 0) {
        return CommentDao::getPreListByObj(get_class($this), $this->id, $cnt, $offsetcommentid);
    }
