...
/**
 * IDGeneratorByDb,StubIDGenerator
 * @desc		ID生成器
 * @remark		依赖类: DbExecuter , BeanFinder
 * @copyright	(c)2012 xwork.
 * @file		IDGeneratorByDb.class.php
...
// 桩
class StubIDGenerator implements IDGeneratorBase
{
...
class IDGeneratorByDb implements IDGeneratorBase
{
    ...
    public function preLoad ($size = 10) {
        ...
        BeanFinder::get("DbExecuter")->executeNoQuery("update idgenerator set nextid=LAST_INSERT_ID(nextid+$size)");
        $nextId = BeanFinder::get("DbExecuter")->queryValue("select LAST_INSERT_ID() as nextid");
