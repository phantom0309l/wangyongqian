...
class DbExecuter
{
    ...
    public static function logImp ($echoSql, $costTime, $rowCnt) {
        ...
        // 默认打开sql日志，如需关闭请修改配置文件debug_sqllog_close = true
        if (! Config::getConfig('debug_sqllog_close', false)) {
        ...
        Debug::addSql($echoSql, $costTime, false);
    }
    ...
    private function execute ($sql, $bind = array()) {
        ...
        try {
            $timeStart = Debug::getCurrentTime();

            // 带编译功能
            $stmt = $this->writeConnection->prepare($sql);
            $stmt->execute($bind);
            $result = $stmt->rowCount();
            $costTime = Debug::getCostTime($timeStart);
    ...
    public function queryImp ($sql, $bind = array()) {
        ...
        $startTime = Debug::getCurrentTime();
        ...
        $costTime = Debug::getCostTime($startTime);
        $echoSql = self::buildSql($sql, $bind);
    ...
    // 往表中插入数据，例
    // BeanFinder::get('DbExectuer')->insert('users',array('username'=>'Lily','age'=>'56'));
