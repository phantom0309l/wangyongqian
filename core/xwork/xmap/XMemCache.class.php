...
class XMemCache
{
    ...
    public static function getOneInstance () {
        ...
        if ($m->set("test-server-status", "ok")) {
            Debug::sys("[-- XMemCache start success. --]");
            return $m;
        } else {
            Debug::sys("[-- XMemCache start false! set cacheOpen=false. --]");
    ...
    public function __construct () {
        ...
        foreach ($memCachedConfigs as $memCachedConfig) {
            $str = "[-- XMemCache addServer " . $memCachedConfig['host'] . ':' . $memCachedConfig['port']." --]";
            Debug::sys($str);
    ...
    public function delete ($key) {
        ...
        if (! empty($ret)) {
            Debug::sys("[-- XMemCache delete of $key --]");
    ...
    public function write ($key, $value, $compression = false, $expireTime = 86400) {
        ...
        // Config::setConfig("xmemcachelog",$xmemcachelog);
        // Debug::sys("[XMemCache::write][end] --");
    }

    public function get ($key) {
        $cacheNeedReload = Config::getConfig("cacheNeedReload", false);
        if ($cacheNeedReload) {
            Debug::sys("[-- cacheNeedReload : XMemCache get 0 of $key --]");
            return null;
        }
        ...
        if (empty($ret)) {
            Debug::sys("[-- XMemCache get 0 of $key --]");
            $ret = null;
        } else {
            Debug::sys("[-- XMemCache get 1 of $key --]");
        }
        return $ret;
    }
    ...
    public function set ($key, $value, $compression = false, $expireTime = 600) {
        ...
        if (! empty($ret)) {
            ...
            Debug::sys("[-- XMemCache set of $key --]");
    ...
    public function replace ($key, $value, $compression = false, $expireTime = 600) {
        Debug::sys("[-- XMemCache replace of $key --]");
    ...
    public static function flushServers () {
        Debug::sys("[-- XMemCache::flushServers() --]");
        $memCachedConfigs = Config::getConfig("mem_cached_cluster");
        foreach ($memCachedConfigs as $memCachedConfig) {
            $memcache_obj = new Memcache();
            @$memcache_obj->connect($memCachedConfig['host'], $memCachedConfig['port']);
    ...
    public static function setCache ($key, $value, $compression = false, $expireTime = 4000) {
        ...
        $key = self::genKey($key);
        if (empty($value)) {
            Debug::sys("[-- XMemCache setCache empty of {$key} --]");
            return true;
        }

        Debug::sys("[-- XMemCache setCache 1 , time={$expireTime} of {$key} --]");
        $xMemCache = BeanFinder::get("XMemCache");
        return $xMemCache->set($key, $value, $compression, $expireTime);
    }
    ...
    public static function getCache ($key) {
        ...
        $key = self::genKey($key);
        $xMemCache = BeanFinder::get("XMemCache");
        $value = $xMemCache->get($key);

        if (empty($value)) {
            Debug::sys("[-- XMemCache getCache 0 of {$key} --]");
        } else {
            Debug::sys("[-- XMemCache getCache 1 of {$key} --]");
        }
