...
class Net_Curl
{
    ...
    /**
     * Time allowed for current transfer, in seconds.
     * 0 means no limit
     *
     * @var int $timeout
     * @access public
    ...
    /**
     * The type of transfer to perform (ie.
     * 'POST', 'GET', 'PUT', etc)
     *
     * @var string $type
     * @access public
     ...
     *
     * @param $url string
     *            The URL to fetch (can be set using the $url property as well)
    ...
    function execute () {
        ...
        if (is_numeric($this->verifyHost) && $this->verifyHost >= 0 && $this->verifyHost <= 2) {
        ...
        if ($this->mute !== true) {
            //$ret = @curl_setopt($this->_ch, CURLOPT_MUTE, false);//removed in curl 7.15.5 now is 7.35.0
        }
        ...
        // todo by sjp 注释掉了此行
            ...
            // '.$info['http_code']); // todo by sjp 注释掉了此行
            }
        }
