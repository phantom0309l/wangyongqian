        ...
        if (!jrac_loaded) {
          ...
          .slider({
            ...
            slide: function(event, ui) {
              var ratio = ui.value / $zoom_widget.on_start_width_value;
              var height = Math.round($zoom_widget.on_start_height_value * ratio);
              $image.height(height);
              ...
              $viewport.observator.notify('jrac_image_width', ui.value);
                //$('.jrac_crop').each(function() {
                    //var l = $(this).offset().left;
                    //var t = $(this).offset().top;
                    //$(this).offset({left:Math.round(l*ratio), top:Math.round(t*ratio)});
                //});
          ...
          $image.draggable({
            ...
            stop: function(event, ui) {
                dealtaLeft = ui.helper.offset().left - imageOffset.left;
                dealtaTop = ui.helper.offset().top - imageOffset.top;
                $('.jrac_crop').each(function() {
                    var l = $(this).offset().left;
                    var t = $(this).offset().top;
                    $(this).offset({left:l + dealtaLeft, top:t+dealtaTop});
                });
                $viewport.observator.notify('jrac_crop_x', $viewport.observator.crop_position_x());
                $viewport.observator.notify('jrac_crop_y', $viewport.observator.crop_position_y());
            }
