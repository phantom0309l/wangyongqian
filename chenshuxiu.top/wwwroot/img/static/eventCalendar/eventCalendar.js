    ...
    Calendar.prototype = {
        ...
        showCalendar: function () { // 输入数据并显示
            ...
            this.$calendarDate_item.each(function (i) {
                ...
                var jItem = $(this);
                if (allDay.getDay() == self.opts.day) {
                    var dt1 = self.fixStartDate.getTime();
                    var dt2 = allDay.getTime();
                    if (dt2 > dt1) {
                        var week = parseInt(Math.abs(dt1 - dt2) / 1000 / 60 / 60 / 24 / 7);
                        // console.log("第" + week + "周", allDay_str);
                        var box = $("<p style='margin-bottom: 0; margin-top: -18px;'></p>");
                        self.opts.rules.forEach(function (value, index, array) {
                            var start = value.range.start;
                            var end = value.range.end;
                            if (week < start || (end != "∞" &&  week > end)) {   // 超出范围
                                return false;
                            }
                            if (week % value.interval == 0) {
                                value.items.forEach(function (subValue, subIndex, subArray) {
                                    var ename = subValue.ename;
                                    var dot = "<i class='dot dot-" + ename + "'></i>";
                                    box.append(dot);
                                })
                            }
                        })
        ...
        inital: function () { // 初始化
            ...
            var startDate = changingStr(this.opts.startDateStr);
            var offset = startDate.getDay() - this.opts.day;
            var startDateValue = startDate.valueOf();
            startDateValue = startDateValue - offset * 24 * 60 * 60 * 1000;
            this.fixStartDate = new Date(startDateValue);
