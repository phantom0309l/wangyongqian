              ...
              <div class="panel-body">
                <div class="col-md-4">
                    <div class="form-group">
                        <label>任务类型</label>
                        <?= HtmlCtr::getSelectCtrImp(CtrHelper::toOptaskTplCtrArray($optasktpls, false),'optasktplid', $optasktplid, 'js-select2 form-control optasktplid')?>
                    </div>
                    <div class="form-group">
                        <label>任务状态</label>
                        <?php
                        echo HtmlCtr::getSelectCtrImp(CtrHelper::toOptaskStatuCtrArray(), "status", $status, 'js-select2 form-control status', 'width: 100%;');
                        ?>
                    </div>
                    <div class="form-group">
                        <label>开始日期</label>
...
$footerScript = <<<STYLE
    $(function(){
        $(".outdata").on("click", function(){
            ...
            var optasktplid = $(".optasktplid option:selected").val();
            var status = $(".status option:selected").val();
            window.location.href = "/rptmgr/opnodelogdataOutput?startdate=" + startdate + "&enddate=" + enddate + "&optasktplid=" + optasktplid + "&status=" + status;
        })
    })
