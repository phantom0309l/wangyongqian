...
class Http
{
    ...
    public static function send ($rq) {
        ...
        isset($rq['data']) && in_array($method, array(
            'POST',
            'PUT')) && curl_setopt($curlHandle, CURLOPT_POSTFIELDS, $rq['data']);
        ...
        if (isset($rq['cert'])) {
            ...
            if ($ssl) {
                curl_setopt($curlHandle, CURLOPT_SSL_VERIFYPEER, false); // true
                                                                        // any
                                                                        // ca
                curl_setopt($curlHandle, CURLOPT_SSL_VERIFYHOST, 1); // check
                                                                    // only host
                if (isset($rq['ssl_version'])) {
                    curl_setopt($curlHandle, CURLOPT_SSLVERSION, $rq['ssl_version']);
                } else {
