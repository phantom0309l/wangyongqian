...
class YouTu
{
    ...
    public static function detectface ($image_path, $isbigface) {
        ...
        if (! file_exists($real_image_path)) {
            return array(
                ...
                'message' => 'file ' . $image_path . ' not exists',
                'data' => array());
        }
     ...
     *
     * @param
     *            image_path_a 待比对的A图片数据
     * @param
     *            image_path_b 待比对的B图片数据
     * @return 返回的结果，JSON字符串，字段参见API文档
     */
    public static function facecompare ($image_path_a, $image_path_b) {
        ...
        if (! file_exists($real_image_path_a)) {
            return array(
                'httpcode' => 0,
                'code' => self::HTTP_BAD_REQUEST,
                'message' => 'file ' . $image_path_a . ' not exists',
                'data' => array());
        }
        ...
        if (! file_exists($real_image_path_b)) {
            return array(
                'httpcode' => 0,
                'code' => self::HTTP_BAD_REQUEST,
                'message' => 'file ' . $image_path_b . ' not exists',
                'data' => array());
        }
     ...
     * @param
     *            image_path_arr 待增加的包含人脸的图片数据，可加入多张（包体大小<2m）
     * @param
     *            facetag 人脸备注信息，用户自解释字段
     * @return 返回的结果，JSON字符串，字段参见API文档
     */
        ...
        foreach ($image_path_arr as $one_path) {
            if (! file_exists($one_path)) {
                return array(
                    'httpcode' => 0,
                    'code' => self::HTTP_BAD_REQUEST,
                    'message' => 'file ' . $one_path . ' not exists',
                    'data' => array());
            }
     ...
     *            person_id 新增人脸的个体身份id
     * @param
     *            url_arr 待增加的包含人脸的图片url，可加入多张（包体大小<2m）
     * @param
     *            facetag 人脸备注信息，用户自解释字段
     * @return 返回的结果，JSON字符串，字段参见API文档
     */
