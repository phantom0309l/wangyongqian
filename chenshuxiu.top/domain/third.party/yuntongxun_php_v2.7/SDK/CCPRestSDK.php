<?php
/*
 * Copyright (c) 2014 The CCP project authors. All Rights Reserved. Use of this
 * source code is governed by a Beijing Speedtong Information Technology Co.,Ltd
 * license that can be found in the LICENSE file in the root of the web site.
 * http://www.yuntongxun.com An additional intellectual property rights grant
 * can be found in the file PATENTS. All contributing project authors may be
...
class CcpREST
{
    ...
    private $Filename = "/home/xworklog/CCPRestLog/log.txt"; // 日志文件
    private $Handle;
     ...
     *            maxCallTime 最大通话时长
     * @param
     *            hangupCdrUrl 实时话单通知地址
     * @param
     *            alwaysPlay 是否一直播放提示音
     * @param
     *            terminalDtmf 用于终止播放promptTone参数定义的提示音
     * @param
     *            needBothCdr 是否给主被叫发送话单
     * @param
     *            needRecord 是否录音
     * @param
     *            countDownTime 设置倒计时时间
     * @param
     *            countDownPrompt 倒计时时间到后播放的提示音
     */
    function callBack ($from, $to, $customerSerNum, $fromSerNum, $promptTone, $alwaysPlay, $terminalDtmf, $userData, $maxCallTime, $hangupCdrUrl, $needBothCdr,
     ...
     * @param
     *            respUrl 外呼通知状态通知回调地址，云通讯平台将向该Url地址发送呼叫结果通知。
     * @param
     *            userData 用户私有数据
     * @param
     *            maxCallTime 最大通话时长
     ...
     *
     * @param
     *            callSid 一个由32个字符组成的电话唯一标识符
     * @param
     *            type 0： 任意时间都可以挂断电话；1 ：被叫应答前可以挂断电话，其他时段返回错误代码；2：
